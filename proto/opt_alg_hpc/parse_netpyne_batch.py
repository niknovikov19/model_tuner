...
dirpath_batch = r'D:\WORK\Salvador\repo\model_tuner\models\L24\exp_results\rx_batch_unconn_1'
exp_name = 'rx_batch_unconn_1'

pop_names = ['L2e', 'L2i', 'L4e', 'L4i']

par_name = 'ext_inp_rate_common'


class BatchMetricGetter1D:
    ...
    def get_batch_param_names(self) -> List[str]:
        return list(self._batch_params.keys())
    
    def get_batch_param_values(self, param_name: str) -> List[Any]:
        return self._batch_param_values[param_name]
    ...
    def _parse_batch_result(
        ...
        os.makedirs(dirpath_batch, exist_ok=True)
    ...
    def get_pop_rates_batch(self, pop_name: str) -> List[float]:
        return [net_rates.data[pop_name] for net_rates in self._net_rates_list]

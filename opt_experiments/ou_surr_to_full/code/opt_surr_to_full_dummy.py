...
# Experiment name
exp_name = 'test_dummy_vsigma_1.5'
...
# Local folders to store intermediate optimization plots
...
def dummy_Ru_Rc_map(Ru_lst: NetRegime1DList) -> NetRegime1DList:
    Rc_lst = NetRegime1DList()
    for Ru in Ru_lst:
        Rc = NetRegime1D(Ru, force_copy=True)
        for pop in Rc.get_pop_names():
            Rc[pop].value *= 0.5
        Rc_lst.append(Rc)
    return Rc_lst
...
with SSHClient(ssh_par_fs=ssh_par_fs, ssh_par_conn=ssh_par_conn) as ssh:
    ...
    while iter_num < n_iter:
        ...
        for n, Ru in enumerate(Ru_lst):
            ...
            # Dummy mode: don't generate any requests
            continue
        ...
        fpath_info = dirpath_info / f'Ru_Rc_{iter_num}.pkl'
